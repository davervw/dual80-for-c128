...
The regular 40-column screen (VIC-II) is on the left, and the normally 80-column screen (VDC) is on the right, but reconfigured to display only rightmost 40 columns and IRQ copies what KERNAL stores there on left side but hidden to the VIC-II screen in 5 line increments every 1/60 second when VDC not busy, so at most 12FPS
